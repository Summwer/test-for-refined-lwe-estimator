...
******************************
Test for Refined LWE Estimator
******************************

First compile the project following the guidance in `The General Sieve Kernel (G6K)`, i.e. running 
.. code-block:: bash

    # once only: creates local python env, builds fplll, fpylll and G6K
    ./bootstrap.sh  -j  4 #4 is the threads for compiling.
    
    # for every new shell: activates local python env
    source ./activate                   


We test the success probability for the estimated dsvp and the test data(`actual` value in Fig.2) is generated by running the command. The parameters `blocksizes` and `dsvp` should be generated from the implemented file `sage/lwechal-est/lwechal-two-step.sage` in [https://github.com/Summwer/lwe-estimator-with-pnjbkz](https://github.com/Summwer/lwe-estimator-with-pnjbkz) by inputting the corresponding $n$ and $\alpha$. Then, setting the `blocksizes` and `dsvp` in `lwe_prob_test.py` file.

.. code-block:: bash

    python lwe_prob_test.py
